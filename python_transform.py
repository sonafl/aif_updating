...
class query_CH_test(TransformationSpark):
    ...
    def execute(self):
        ...
        def inCatz(zz):  
           client.execute('INSERT INTO {new_catalog} ("IsMaster", "hashKey", "hashPRJKey", "OriginalItemID", "catalogid", "FullTitle", "MasterTitle", "IsSeries", "Project", "SeasonNum", "EpisodeNum", "ProdYear", "ProdCountry", "isoCountry", "uploadtime", "editor", "IsTrailer", "Duration", "ReleaseDate") VALUES', zz)
        def inLog(zz): 
           client.execute('INSERT INTO {update_log} ("editor", "partner_name", "catalogid", "count_OriginalItemID", "uploadtime") VALUES', zz)
        
        logger.info('Готовим страны')
        countrys = {}
        qс = "SELECT iso, name FROM {country}"
        ...
        qw = "SELECT DISTINCT ProdCountry AS Country FROM {local_art} WHERE ProdCountry != ' ' "
        ...
        def GetContacts2(day, catalogid): 
            ...
            dfp = pd.DataFrame(lst, columns=["OriginalItemID", "catalogid", "FullTitle", "Title", "IsSeries", "Project", "SeasonNum", "EpisodeNum", 
                                            "ProdYear", "ProdCountry"])
            return dfp
        ...
        def GetKP():  
            q = "with \
            KP AS ( SELECT  cu_id, MAX(upload_datetime) as uploadtime FROM {kp_base} group by  cu_id ) \
        ...
        def GetCat(catalogid): 
            ...
            q = "WITH FA AS ( SELECT DISTINCT OriginalItemID, CatalogID, MAX(LastUpdateDate) AS uploadtime \
            FROM {local_art} \
            WHERE CatalogID = "+str(catalogid)+"  AND OriginalItemID NOT LIKE 'adv_%' \
            AND LastUpdateDate > '"+str(tdays)+"' \
            GROUP BY OriginalItemID, CatalogID) \
        ...
        def getNotISO(prj, pr, catid): 
            qсp = "with \
            NU AS ( \
            SELECT OriginalItemID, MAX(uploadtime) as uploadtime FROM {new_catalog} where  catalogid = "+str(catid)+" group by OriginalItemID ) \
            SELECT OriginalItemID, Project, ProdYear \
            FROM {new_catalog} as NC \
        ...
        def isomain(_ProdYear, Project, Country, iso, catid):
            q = "with \
            NU AS ( \
            SELECT OriginalItemID, MAX(uploadtime) as uploadtime FROM {new_catalog} group by OriginalItemID ) \
        ...
        def MakeTrailers(catalogid):  
            ...
            q = "WITH \
            N AS  ( SELECT Project, OriginalItemID, IsTrailer, MAX(uploadtime) AS updatedate FROM {new_catalog} WHERE catalogid = "+str(catalogid)+"  GROUP BY Project, OriginalItemID, IsTrailer ), \
            T AS  ( SELECT ContentUnitID, MAX(upload_datetime) AS uploaddatetime FROM {trailers} WHERE  IsTrailer = TRUE  and ContentUnitCatalogID = '"+str(catalogid)+"' GROUP BY ContentUnitID), \
            TT AS ( SELECT ContentUnitID, IsTrailer FROM {trailers} AS TM JOIN T ON T.ContentUnitID = TM.ContentUnitID AND T.uploaddatetime = TM.upload_datetime WHERE  IsTrailer = TRUE  and ContentUnitCatalogID = '"+str(catalogid)+"' GROUP BY ContentUnitID, IsTrailer) \
            SELECT distinct TT.IsTrailer, NC.* \
            FROM {new_catalog} AS NC JOIN N ON N.OriginalItemID = NC.OriginalItemID AND N.updatedate = NC.uploadtime JOIN TT ON NC.OriginalItemID = TT.ContentUnitID \
        ...
        def MakeNewTrailers(catalogid):  
            ...
            q = "WITH N AS  ( SELECT OriginalItemID, MAX(uploadtime) AS updatedate FROM {new_catalog} WHERE catalogid = "+str(catalogid)+"  GROUP BY OriginalItemID ) \
        ...
        def MakeOnlyNewTrailers(catalogid):  
            ...
            q = "WITH \
            N AS  ( SELECT OriginalItemID, MAX(uploadtime) AS updatedate FROM {new_catalog} WHERE catalogid = "+str(catalogid)+"  GROUP BY OriginalItemID ), \
            T AS  ( SELECT ContentUnitID, MAX(upload_datetime) AS uploaddatetime FROM {trailers} WHERE  ContentUnitCatalogID = '"+str(catalogid)+"' GROUP BY ContentUnitID), \
            TT AS ( SELECT ContentUnitID, IsTrailer, TM.upload_datetime FROM {trailers} AS TM JOIN T ON T.ContentUnitID = TM.ContentUnitID AND T.uploaddatetime = TM.upload_datetime \
            WHERE  ContentUnitCatalogID = '"+str(catalogid)+"' GROUP BY ContentUnitID, IsTrailer, upload_datetime ) \
            SELECT distinct TT.IsTrailer, NC.* \
            FROM {new_catalog} AS NC JOIN N ON N.OriginalItemID = NC.OriginalItemID AND N.updatedate = NC.uploadtime JOIN TT ON NC.OriginalItemID = TT.ContentUnitID \
